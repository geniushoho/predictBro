...
date_time_obj = datetime.datetime.now().date()
...
#    print(year0+month0+day0)
    ...
    h = requests.get(url, verify=False, headers={'Connection':'close'})
    time.sleep(5)
#    print(h.text)
#    print(len(h.text))
#    print("test")
...
#        print(j)
        ...
        print(yearb2+monthb2+dayb2)
        print(year0+month0+day0)
        res = requests.get(url, verify=False, headers={'Connection':'close'})
...
#df=df.loc['date']  #选择表格中的'w'、'z'列
A=(df.loc[17,'bmoney']/df.loc[17,'bnum'])-(df.loc[14,'bmoney']/df.loc[14,'bnum'])  #选择表格中的'w'、'z'列
B=(df.loc[17,'pmoney']/df.loc[17,'pnum'])-(df.loc[14,'pmoney']/df.loc[14,'pnum'])  #选择表格中的'w'、'z'列
Ab1=(df.loc[11,'bmoney']/df.loc[11,'bnum'])-(df.loc[8,'bmoney']/df.loc[8,'bnum'])  #选择表格中的'w'、'z'列
Bb1=(df.loc[11,'pmoney']/df.loc[11,'pnum'])-(df.loc[8,'pmoney']/df.loc[8,'pnum'])  #选择表格中的'w'、'z'列
Ab2=(df.loc[5,'bmoney']/df.loc[5,'bnum'])-(df.loc[2,'bmoney']/df.loc[2,'bnum'])  #选择表格中的'w'、'z'列
Bb2=(df.loc[5,'pmoney']/df.loc[5,'pnum'])-(df.loc[2,'pmoney']/df.loc[2,'pnum'])  #选择表格中的'w'、'z'列
...
################ OP Ratio ########################
date2=df.loc[2,'date']
BCmoney2=df.loc[2,'bmoney']
BPmoney2=df.loc[5,'bmoney']
SCmoney2=df.loc[2,'pmoney']
SPmoney2=df.loc[5,'pmoney']
...
date1=df.loc[8,'date']
BCmoney1=df.loc[8,'bmoney']
BPmoney1=df.loc[11,'bmoney']
SCmoney1=df.loc[8,'pmoney']
SPmoney1=df.loc[11,'pmoney']
...
date0=df.loc[14,'date']
BCmoney0=df.loc[14,'bmoney']
BPmoney0=df.loc[17,'bmoney']
SCmoney0=df.loc[14,'pmoney']
SPmoney0=df.loc[17,'pmoney']
...
if ((not(hweektoday != 5)) and (not(hweektoday != 6)) and (0 <= int("%d" % datetime.datetime.now().hour) < 15)):
     ...
     picURL = 'https://inews.gtimg.com/newsapp_match/0/11101132122/0'
     ...
     picURL = 'https://0.soompi.io/wp-content/uploads/2016/06/27135606/TWICE-Tzuyu.png?s=900x600&e=t'
...
#ORI PIC#  picURL = 'https://i2.kknews.cc/SIG=398a9vi/1o800006q7o21nr6r25r.jpg'
